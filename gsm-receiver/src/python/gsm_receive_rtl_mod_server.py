...
#Copyright 2015 Paul Kinsella <kali_gsm_rtl_sdr@gmail.com>
...
#
#############################
def setServerStatus(setstate):
	global SERVER_RUNNING
        SERVER_RUNNING = setstate# False/True
...
class top_block(grc_wxgui.top_block_gui):
  ...
  def __init__(self):
    ...
    def startServerThread(void):
         if getServerStatus() == False:
 		try:
			thread.start_new_thread(startServer,(self.edit_server_ip.GetValue(),int(self.edit_server_port.GetValue()),),)
            		self.btnstart_server.SetLabel("Stop Server")
	    		setServerStatus(True);
			self.serverlabel_state.SetLabel("Waiting for client...")
		except:
    ...
    def startServer(NEW_IP,NEW_PORT):
	S_C = '$'
	COMMAND = ""
	E_C = '@'
	BUFFER_SIZE = 64
	IP = NEW_IP#self.edit_server_ip.GetValue()
	PORT = NEW_PORT#int(self.edit_server_port.GetValue())
	
	s.bind((IP,PORT))
	s.listen(5)
	conn,addr = s.accept()
	#self.serverlabel_state.SetLabel("Connected:"+str(addr))
	print 'Connected',addr

	while getServerStatus() == True:
		data = conn.recv(BUFFER_SIZE)
		if data == 'CLOSES' : break#Close if this string appears
		if data != "" and '$' in data and '@' in data:
			print "COMMAND GOT:",data[data.index(S_C)+1:data.index(E_C)]
			print "Recv data",data
			COMMAND_C = data[data.index(S_C)+1:data.index(E_C)]

			if COMMAND_C == 'FREQ1':
				print "set freq 1"
				incfreq("")
			elif COMMAND_C == 'FREQ2':
				print "set freq 2"
				decfreq("")
			elif 'SETFREQ:' in COMMAND_C:
				#"Set with $SETFREQ:944.55e6@"
				freqfloat = COMMAND_C.replace("SETFREQ:","")
				freqfloat +="e6"
				newfreq = float(freqfloat)#ast.literal_eval(freqfloat)
				inctcpfreq(newfreq)

			elif COMMAND_C == 'INCPPM:':
				print "inc ppm"
				pass
			elif COMMAND_C == 'DECPPM':
				print "dec ppm"
				pass
			elif 'SETPPM:' in COMMAND_C:
				print "set ppm"
				pass

			elif COMMAND_C == 'INCGAIN':
				print "inc gain"
				pass
			elif COMMAND_C == 'DECGAIN':
				print "dec gain"
				pass

			elif 'AUTOSETGAIN_ON' in COMMAND_C:
				set_iagc(True)

			elif 'AUTOSETGAIN_OFF' in COMMAND_C:
				set_iagc(False)

			elif 'SETGAIN' in COMMAND_C:
				gain = COMMAND_C.replace("SETGAIN:","")#+"e6"
				newgain = float(gain)
				set_rfgain(newgain)


			elif COMMAND_C == 'INCSAMP_RATE:':
				print "inc sample rate"
				pass
			elif COMMAND_C == 'DECSAMP_RATE:':
				print "dec sample rate"
				pass
			elif 'SETSAMPRATE:'in COMMAND_C:
				sampr = COMMAND_C.replace("SETSAMPRATE:","")#+"e6"
				samprf = float(sampr)
				set_samp_rate(samprf)
				

			elif 'SETKEY:'in COMMAND_C:
				print "set key"
				pass#self.channel_combo_box.get_value()

			elif 'SETCHANNEL:'in COMMAND_C:
				print "set channel"# command to set $SETCHANNEL:2S@
				self.channel_combo_box.set_value(COMMAND_C.replace("SETCHANNEL:",""))
				time.sleep(0.2)
				#resetapp("") 

			elif 'RESETAPP' in COMMAND_C:
				#conn.close()
				resetapp("") 
...
#range loop ends here
    ...
    def OnButtonMaxTimer(void):
        ...
        if btnLabel == "O":#self.btnstart_server
    ...
    combo_channels = ['0B','0C','1S', '2S', '3S', '4S','5S','6S','7S','1T', '2T', '3T', '4T','5T','6T','7T']
    ...
    #	server controls
    ##############################################################
    self.serverlabel = wx.StaticText(debugPanel, label="Server Settings",pos=(495,2),size=(-1,20))
    self.serverlabel_state = wx.StaticText(debugPanel, label="Not Connected:",pos=(615,2),size=(-1,20))

    self.serverlabel_ip = wx.StaticText(debugPanel, label="IP:",pos=(495,25),size=(-1,20))
    self.edit_server_ip = wx.TextCtrl(debugPanel,value="192.168.43.176",size=(140,25),pos=(520,25))

    self.serverlabel_port = wx.StaticText(debugPanel, label="PORT:",pos=(660,25),size=(-1,20))
    self.edit_server_port = wx.TextCtrl(debugPanel,value="5555",size=(60,25),pos=(710,25))

    self.btnstart_server = wx.Button(debugPanel, label="Start Server", pos=(495,55),size=(110,25))
    self.btnstart_server.Bind(wx.EVT_BUTTON,startServerThread)
